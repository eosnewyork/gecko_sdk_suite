...
static RAIL_RxPacketDetails_t *MAIN_rxPacket;
...
void handleReceivedPacket()
{
  ...
  printf("-------------------- RX packet --------------------\r\n");
  printf("Length      : TODO\r\n");
  printf("RSSI        : %d [dBm]\r\n", RAIL_GetRssi(railHandle));
  printf(".time       : %ld [us]\r\n", MAIN_rxPacket->timeReceived.packetTime);
  printf(".crcStatus  : %s\r\n", (MAIN_rxPacket->crcPassed) ? "OK" : "Invalid");
  printf(".lqi        : %d\r\n", MAIN_rxPacket->lqi);
  printf(".syncWordId : %d\r\n", MAIN_rxPacket->syncWordId);
  printf(".rssiLatch  : %d [dBm]\r\n", (int8_t)MAIN_rxPacket->rssi);
  printf("Data   : %02X %02X %02X %02X %02X %02X %02X %02X\r\n",
         MAIN_rxBuffer[0], MAIN_rxBuffer[1],
         MAIN_rxBuffer[2], MAIN_rxBuffer[3],
         MAIN_rxBuffer[4], MAIN_rxBuffer[5],
         MAIN_rxBuffer[6], MAIN_rxBuffer[7]);
#endif

  if ( MAIN_rxBuffer[0] == PACKET_TYPE_FREEFALL
       || MAIN_rxBuffer[0] == PACKET_TYPE_MOVEMENT ) {
    /* One of the other nodes is moving, use their color */

    red   = MAIN_rxBuffer[2];
    green = MAIN_rxBuffer[3];
    blue  = MAIN_rxBuffer[4];
    BOARD_rgbledSetColor(red, green, blue);
    dontDimIterations = MOVEMENT_TIMEOUT;
    lastActiveIteration = loopCount;
    dimIterations = 0;
  } else if ( MAIN_rxBuffer[0] == PACKET_TYPE_DIM_NODE
              && nodeRole == NODE_ROLE_REGULAR
              && dontDimIterations == 0 ) {
    /* All nodes are stationary and the master node is broadcasting DIM packets */

    if ( (MAIN_rxBuffer[1] == nodeID) ) {
      if (dimIndex != nodeID) {
        /* Start dimming now */
        dimIterations = DIM_ITERATIONS;
        dimIndex = nodeID;
...
void initIMU(void)
{
  ...
  if ( status != ICM20648_OK ) {
    printf("Could not get IMU device id! status = %08lXh\r\n", status);
    return;
  }
...
static void configAccel(void)
{
  ...
  printf("IMU sample rate set to %d Hz\r\n", (uint16_t)accelSampleRate);
