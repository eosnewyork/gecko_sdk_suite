...
static void setupPeriodicTx(void);
static void periodicTimerCallback(RTCDRV_TimerID_t id, void *user);
...
static RTCDRV_TimerID_t periodicTimer = 0;
...
{
  if (RTCDRV_StartTimer(periodicTimer,
...
{
  if (RTCDRV_StopTimer(periodicTimer)) {
    while (1) ;
  }
}
...
static void setupPeriodicTx(void)
{
  if (RTCDRV_AllocateTimer(&periodicTimer)) {
    while (1) ;
  }
}
