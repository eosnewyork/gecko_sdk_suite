...
void GRAPHICS_DrawInitScreen(GLIB_Context_t *pContext)
{
  ...
  sprintf(buff,
          ...
          (long int)(RAIL_GetTxPowerDbm(railHandle) / 10));
...
uint8_t GRAPHICS_DrawConstInfo(GLIB_Context_t *pContext, uint8_t row)
{
  ...
  if (RADIO_MODE_TX == rangeTest.radioMode) {
    ...
    if ( 0u < (channelConfigs[rangeTest.phy]->configs[0u].baseFrequency % 1000000ul)) {
      sprintf(buff,
              "%u.%02uMHz",
              (uint16_t) (channelConfigs[rangeTest.phy]->configs[0u].baseFrequency / 1000000ul),
              (uint16_t) ((channelConfigs[rangeTest.phy]->configs[0u].baseFrequency % 1000000ul) / 10000u));
    } else {
      sprintf(buff,
              "%uMHz",
              (uint16_t) (channelConfigs[rangeTest.phy]->configs[0u].baseFrequency / 1000000ul));
    ...
    sprintf(buff, "%ukHz", (uint16_t) (channelConfigs[rangeTest.phy]->configs[0u].channelSpacing / 1000u));
    ...
    sprintf(buff,
            "Fch:%4ukHz  Ch#: %3u",
            (uint16_t) (channelConfigs[rangeTest.phy]->configs[0u].channelSpacing / 1000u),
            rangeTest.channel);
    GLIB_drawString(pContext, buff, strlen(buff), 1u, row, false);
    row += GRAPHICS_FONT_HEIGHT;

    if (RADIO_MODE_TRX == rangeTest.radioMode) {
      RAIL_TxPower_t power = RAIL_GetTxPowerDbm(railHandle);
      sprintf(buff, "P:%+lidBm", (long int)(power / 10));
    }
    GLIB_drawString(pContext, buff, strlen(buff), 1u, row, false);
    row += GRAPHICS_FONT_HEIGHT;
...
uint8_t GRAPHICS_DrawVarInfo(GLIB_Context_t *pContext, uint8_t row)
{
  ...
  if (RADIO_MODE_TX == radioMode) {
    ...
    if (RANGETEST_PACKET_COUNT_INVALID == pktsRcvd) {
      ...
      char tmpMaStr[10u] = { 0 };
      char tmpPerStr[10u] = { 0 };

      if (MA < 10.0f) {
        tmpMaStr[0] = ' ';
        tmpMaStr[1] = ' ';
        sprintfFloat(&tmpMaStr[2], 8, MA, 1);
      } else if (MA < 100.0f) {
        tmpMaStr[0] = ' ';
        sprintfFloat(&tmpMaStr[1], 9, MA, 1);
      } else {
        sprintfFloat(tmpMaStr, 10, MA, 1);
      }

      if (PER < 10.0f) {
        tmpPerStr[0] = ' ';
        tmpPerStr[1] = ' ';
        sprintfFloat(&tmpPerStr[2], 8, PER, 1);
      } else if (PER < 100.0f) {
        tmpPerStr[0] = ' ';
        sprintfFloat(&tmpPerStr[1], 9, PER, 1);
      } else {
        sprintfFloat(tmpPerStr, 10, PER, 1);
      }

      sprintf(buff, "MA:%s%%  PER:%s%%", tmpMaStr, tmpPerStr);
    }
    GLIB_drawString(pContext, buff, strlen(buff), 1u, row, false);
    row += GRAPHICS_FONT_HEIGHT;
  }
...
void GRAPHICS_DrawTRX(GLIB_Context_t *pContext)
{
  ...
  //Check if frequency has fractional part in MHz scale
  if (channelConfigs[rangeTest.phy]->configs[0u].baseFrequency % 1000000ul) {
    sprintf(buf,
            "%uM%02u",
            (uint16_t) (channelConfigs[rangeTest.phy]->configs[0u].baseFrequency / 1000000ul),
            (uint16_t) ((channelConfigs[rangeTest.phy]->configs[0u].baseFrequency % 1000000ul) / 10000u));
