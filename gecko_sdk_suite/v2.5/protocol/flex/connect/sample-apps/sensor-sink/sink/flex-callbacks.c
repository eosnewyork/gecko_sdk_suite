...
void removeChildCommand(void)
{
  EmberStatus status;
  EmberMacAddress address;

  address.mode = emberUnsignedCommandArgument(0);

  if (address.mode == EMBER_MAC_ADDRESS_MODE_SHORT) {
    address.addr.shortAddress = emberUnsignedCommandArgument(1);
  } else {
    emberCopyEui64Argument(2, address.addr.longAddress);
  }

  status = emberRemoveChild(&address);

  emberAfCorePrintln("Child removal 0x%x", status);
}
