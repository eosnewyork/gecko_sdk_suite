 ...
 *  run the events associated with that task, it should call ::emberRunTask()
 ...
 *  within that time.
 *  NOTE: If any events are modified within an interrupt, to guarantee
...
/** @brief Does the same as emberMsToNextEvent() with the
 *  following addition.  If the returnIndex is non-NULL, it sets the value
 *  pointed to by the pointer to equal the index of the event that is
 *  ready to fire next.  If no events are active, it returns 0xFF.
 */
...
/** @brief Initializes a task for managing events and processor
 *  idling state.
 ...
 *   because a task has something to do.
 */
bool emberMarkTaskIdle(EmberTaskId taskid);

#ifndef EMBER_NO_IDLE_SUPPORT
/** @brief Indicates that an application supports processor idling.
 */
