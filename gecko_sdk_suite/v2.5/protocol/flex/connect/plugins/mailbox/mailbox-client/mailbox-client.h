...
/** @brief Submits a data message to a mailbox server. If this
...
/** @brief Queries a mailbox server for pending messages. If this
 * API returns an ::EmberAfMailboxStatus value of
 * ::EMBER_MAILBOX_STATUS_SUCCESS, the corresponding asynchronous callback
 ...
 *  be sent secured. Note that secured pending messages can only be
 *  retrieved performing a secured query while unsecured pending messages can
 *  be retrieved using either an unsecured or a secured query.
 *
 *  @return An ::EmberAfMailboxStatus value of:
 ...
 *
 * A callback invoked after a successful
 ...
 *
 * A callback that may be invoked either if the a message that was submitted
 * to a mailbox server reached its final destination or it timed-out.
 * Note that the callback is not always called. If the status
 ...
 *
 * @param mailboxServer   The mailbox server short ID where the message was
 *   submitted to.
 *
 ...
 *
 *  @param message   A pointer to the retrieved message payload. Note that
 ...
 *  more pending messages for this mailbox client. Note that this parameter
