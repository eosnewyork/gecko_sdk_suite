 ...
 * if ezsp-utils.c is used, sets the size of the source route table on
 * the NCP.
 */
...
/** @brief Defines the size of the receive buffer pool on the EZSP host.
 *
 ...
 * is the maximum number of callbacks that can be received between
 ...
 *
 * The form and join host library use a flat buffer to store channel energy,
 * pan IDs, and matching networks.  The underlying data structure is an
 * uint16_t[], which means that the true storage size is twice this value.  The library requires
 * the buffer to be at least 32 bytes, which means that the minimum size is 16.  A matching
 * network requires 16 to 20 bytes, depending on the structure padding.
 */
