...
void initRadio()
{
  ...
  //PRS clock (and PRS channel 7) is required for RAIL timer sync
...
int main(void)
{
  ...
  state = S_SCHEDULE_TX;
  gLastTxEndTime = RAIL_GetTime();

  uint16_t length;
  while (1) {
    switch (state) {
      case S_SCHEDULE_TX:
        length = WMBUS_SAMPLE_setupFrame(gTxBuffer, gAccessNumber, gAccessibility, 12345, 123, true, false);
      ...
      case S_SLEEP:
      {
        ...
        RAIL_Status_t sleepError = RAIL_Sleep(RAIL_WAKEUP_TIME, &canDeepSleep);
        if ( RAIL_STATUS_NO_ERROR == sleepError ) {
          if ( canDeepSleep ) {
            EMU_EnterEM2(true);
