...
/** @brief  A callback invoked by the OTA Unicast Client plugin when the client
 * starts receiving a new image. The application can choose to start receiving
 * the image or ignore(refuse) it. If the application chooses to receive the
 * image, other images sent out by another server are ignored until the client
...
/** @brief  A callback invoked by the OTA Unicast Bootloader Client plugin when
 * an image segment that is part of an image the application chose to download
 * was received.
 *
 * @param   serverId    The node ID of the server that initiated the new image
 * distribution process.
...
/** @brief  A callback invoked by the OTA Unicast Bootloader Client plugin to
 * indicate that an OTA Unicast Bootloader Server has requested to perform a
