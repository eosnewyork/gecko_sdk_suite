 ...
 *  the user for a specific hardware and interconect. As shipped, this
 ...
 *  and it returns a "no space indication".
 *
 * @return
 * - ::EZSP_SUCCESS
 * _ ::EZSP_NO_TX_SPACE
...
/** @brief Writes all data in the write output buffer to the serial port
 ...
 *
 * @param byte A pointer to a variable where the byte read will be output.
 *
 * @return
 * - ::EZSP_SUCCESS
 * - ::EZSP_NO_RX_DATA
...
/** @brief Returns a number of bytes available to read from the serial port.
 *
 * @param count A pointer to a variable where the byte count will be written.
 *
 * @return
 * - ::EZSP_SUCCESS
 * - ::EZSP_NO_RX_DATA
...
/** @brief Registers a callback that is called whenever the EZSP serial
...
/** @brief Tests to see if all serial transmit data has been shifted
