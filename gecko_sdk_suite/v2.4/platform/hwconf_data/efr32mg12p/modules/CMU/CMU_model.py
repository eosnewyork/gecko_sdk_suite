...
options = {
    ...
    "BSP_CLK_HFXO_CTUNE_TOKEN": {
        "type": "boolean",
        "description": "Calibrate HFXO CTUNE from mfg token, or factory calibrated value if mfg token is missing",
        "dependency": dep.Dependency(platform=dep.Platform.SERIES1, mcu_type=dep.McuType.RADIO),
        "subcategory": "HFXO",
        "longdescription": "Use value stored in manufacturing token in the LOCKBITS page as oscillator tuning value. This requires that the software used supports reading manufacturing tokens. If the manufacturing token is missing, the implementation then checks for the factory calibrated value, and then if that is missing it should fall back to the HFXO CTUNE value defined above.",
        "generate_if_hidden": False,
    },
    "BSP_CLK_HFXO_BOOST": {
        "type": "enum",
